    ...
    {cards.map((card) => {
      ...
      const body = (
        <div className="rounded-xl border border-[#EADBCD] bg-white p-5 shadow-sm transition hover:border-primary/35">
          ...
          <p className="mt-1 text-3xl font-bold text-[#33251D]">{card.value}</p>
        </div>
      );
...
const ItemList = ({
  ...
  <section className="rounded-xl border border-[#EADBCD] bg-white shadow-sm">
    <div className="border-b border-[#EADBCD] px-5 py-4">
      <h2 className="text-sm font-semibold text-[#33251D]">{title}</h2>
    </div>
      ...
      {items.length === 0 ? (
        ...
        items.map((item) => (
          <Link
            key={item.id}
            to={item.href}
            className="flex items-center gap-4 px-5 py-4 transition hover:bg-[#FAF5ED]"
          >
            <span
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium text-[#33251D]">
                {item.title}
              </p>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.9fr)]">
      ...
      <div className="space-y-6">
        <section className="rounded-xl border border-[#EADBCD] bg-white p-5 shadow-sm">
          <h2 className="text-sm font-semibold text-[#33251D]">
            Podstawowe statystyki
          </h2>
          <div className="mt-4 space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-muted-foreground">Aktywne zapytania</span>
              <strong>{data.stats.activeInquiries}</strong>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Wysłane oferty</span>
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">Aktywne zlecenia</span>
          ...
          {data.alerts.length === 0 ? (
            ...
            data.alerts.map((alert) => (
              <div key={alert.id} className="px-5 py-4">
                ...
                <p className="mt-2 text-sm font-medium text-[#33251D]">
                  {alert.label}
                </p>
                <p className="text-xs text-muted-foreground">
                  {alert.companyName}
